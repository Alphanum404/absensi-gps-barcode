    ...
    @endphp
      ...
      @endphp
        ...
        @endphp
          ...
          {{ isset($attendance['event_location']) ? '(' . $attendance['event_location'] . ')' : '' }}
          </td>
        @endif
        @endif
        ...
        <td class="{{ $bgColor }} cursor-pointer text-center text-sm font-medium text-gray-900 dark:text-white">
...
<!-- Pada component attendance-detail-modal.blade.php -->
@if ($currentAttendance['event'] ?? false)
  <div class="mb-3 w-full">
    ...
    value="{{ $currentAttendance['event']['name'] }} ({{ \Carbon\Carbon::parse($currentAttendance['event']['event_date'])->format('d/m/Y') }})"></x-input>
  </div>
  <div class="mb-3 flex w-full gap-3">
    <div class="w-full">
    <x-label for="event_time" value="Waktu Event"></x-label>
    <x-input type="text" id="event_time" disabled
      value="{{ $currentAttendance['event']['start_time'] }} - {{ $currentAttendance['event']['end_time'] }}"></x-input>
    </div>
    @if($currentAttendance['event']['location'] ?? false)
    <div class="w-full">
    <x-label for="event_location" value="Lokasi Event"></x-label>
    <x-input type="text" id="event_location" disabled value="{{ $currentAttendance['event']['location'] }}"></x-input>
    </div>
    @endif
  </div>
@endif